...
import { Container, Typography, Button, Box, List, ListItem, ListItemText, ListItemAvatar, Avatar, TextField, IconButton } from '@mui/material';
import { createAvatar } from '@dicebear/core';
import { avataaars, micah, bottts, adventurer, identicon, initials } from '@dicebear/collection';
...
const DiscussDetails = () => {
    ...
    useEffect(() => {
        const fetchTopic = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/api/discuss/${id}`);
    ...
    const handleAddComment = async () => {
        try {
            const response = await axios.put(`http://localhost:8000/api/discuss/${id}`, {
                content: newComment,
    ...
    return (
        <Container>
            ...
            <Typography variant="subtitle1" gutterBottom>By {topic.author}</Typography>
                    ...
                    <List>
                        {topic.comments.map((comment) => (
                            <ListItem key={comment._id} alignItems="flex-start">
                                <ListItemAvatar>
                                    <Avatar alt={comment.author} src={getCuteAvatar(comment.author)} />
                                </ListItemAvatar>
                                <ListItemText
                                ...
                                {comment.replies.length > 0 && (
                                    <List>
                                        {comment.replies.map((reply) => (
                                            <ListItem key={reply._id} alignItems="flex-start" style={{ paddingLeft: 80 }}>
                                                <ListItemAvatar>
                                                    <Avatar alt={reply.author} src={getCuteAvatar(reply.author)} />
                                                </ListItemAvatar>
                                                <ListItemText
            ...
            <Box mt={3}>
                <Button variant="contained" color="primary" component={Link} to={`/update/${topic._id}`}>Edit</Button>
